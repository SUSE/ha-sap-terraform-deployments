...
locals {
  ...
  nfs_srv_ip        = var.nfs_srv_ip != "" ? var.nfs_srv_ip : cidrhost(local.subnet_address_range, 6)
  ...
  # Check if nfs server has to be created
  nfs_enabled = var.hana_scale_out_shared_storage_type == "nfs" || var.netweaver_shared_storage_type == "nfs" || var.nfs_enabled
  ...
  nfs_os_image        = var.nfs_os_image != "" ? var.nfs_os_image : var.os_image
...
module "common_variables" {
  ...
  netweaver_nfs_share = (
    var.drbd_enabled
    ? "${local.drbd_cluster_vip}:${var.drbd_nfs_mounting_point}/${var.netweaver_sid}"
    : var.netweaver_shared_storage_type == "nfs"
    ? "${local.nfs_srv_ip}:${var.nfs_nfs_mounting_point}/${var.netweaver_sid}"
...
module "nfs_server" {
  ...
  common_variables      = module.common_variables.configuration
  name                  = var.nfs_name
  network_domain        = var.nfs_network_domain == "" ? var.network_domain : var.nfs_network_domain
  region                = var.region
  region_net            = var.region_net
  bastion_host          = module.bastion.public_ip
  nfs_count             = local.nfs_enabled == true ? 1 : 0
  ...
  network_subnet_id     = local.subnet_id
  firewall_internal     = openstack_networking_secgroup_v2.ha_firewall_internal.id
  os_image              = local.nfs_os_image
  nfs_srv_ip            = var.nfs_srv_ip
  host_ips              = [local.nfs_srv_ip]
  nfs_volume_size       = var.nfs_volume_size
  nfs_data_volume_names = var.nfs_data_volume_names
  nfs_mounting_point    = var.nfs_nfs_mounting_point
  on_destroy_dependencies = [
    openstack_networking_secgroup_v2.ha_firewall_internal
  ]
