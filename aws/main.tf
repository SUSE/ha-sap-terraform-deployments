...
# Monitoring: 10.0.0.5
# Hana ips: 10.0.0.10, 10.0.1.11 (hana machines must be in different subnets)
# Hana cluster vip: 192.168.1.10 (virtual ip address must be in a different range than the vpc)
# Netweaver ips: 10.0.2.30, 10.0.3.31, 10.0.2.32, 10.0.3.33 (netweaver ASCS and ERS must be in different subnets)
...
module "monitoring" {
  ...
  sles4sap_images        = var.sles4sap
  subnet_ids             = aws_subnet.hana-subnet.*.id
  host_ips               = [local.hana_cluster_vip]
