...
resource "aws_instance" "drbd" {
  ...
  instance_type               = var.drbd_instancetype == "" ? var.min_instancetype : var.drbd_instancetype
  ...
  volume_tags = {
    Name = "${terraform.workspace}-${var.name}${var.drbd_count > 1 ? "0${count.index + 1}" : ""}"
  }

  tags = {
    Name      = "${terraform.workspace} - ${var.name}${var.drbd_count > 1 ? "0${count.index + 1}" : ""}"
    Workspace = terraform.workspace
    Cluster   = "${terraform.workspace}-${var.name}${var.drbd_count > 1 ? "0${count.index + 1}" : ""}"
  }
}
